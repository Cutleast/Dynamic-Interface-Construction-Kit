...
class FFDec:
    ...
    def export_shapes(self, shape_ids: list[str], outpath: Path, format: str):
        """
        Exports shape with <shape_id> to <outpath>.

        Params:
            shape_ids: list of strings, ids of shapes to export
            outpath: Path, path to folder the shapes get exported to
        """

        self.log.info(f"Exporting shapes...")

        cmd = f"""\
-format shape:{format} \
-selectid {','.join(shape_ids)} \
-export shape "{outpath}" \
"{self.swf_path}"\
"""

        self._exec_command(cmd)

        self.log.info(f"Shapes exported to '{outpath}'.")
